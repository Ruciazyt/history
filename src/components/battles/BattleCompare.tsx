...
import {
  ...
  BATTLE_COMPARE_GRADIENTS,
  STRATEGY_BADGE_COLORS,
  COMPARISON_SUMMARY_COLORS,
  BATTLE_DETAIL_COLORS,
  BATTLE_COMPARE_COLORS,
  BATTLE_COMPARE_VIEW_COLORS,
...
const BattleCompareSide = React.memo(function BattleCompareSide({ battle, side, t }: BattleSideProps) {
  ...
  const opponentColors = side === 'left' ? PARTY_COLORS.defender : PARTY_COLORS.attacker;
    ...
    <div className={`flex-1 bg-gradient-to-b ${eraGradient} rounded-xl p-4 border ${partyColors.border}`}>
      ...
      {(battle.battle?.commanders?.attacker?.length || battle.battle?.commanders?.defender?.length) && (
        <div className={`mt-3 pt-3 border-t ${BATTLE_COMPARE_COLORS.section.divider}`}>
          <div className={`text-xs ${BATTLE_COMPARE_COLORS.commander.label} mb-2`}>{t('battleCompare.commanders')}</div>
          <div className="flex flex-wrap justify-center gap-1">
            ...
            {battle.battle?.commanders?.defender?.map((cmd, i) => (
              <span key={`def-${i}`} className={`px-2 py-0.5 ${opponentColors.badge} ${BATTLE_COMPARE_COLORS.commander.badge}`}>👤 {cmd}</span>
            ))}
          </div>
        </div>
      )}
      ...
      <div className={`mt-4 pt-4 border-t ${BATTLE_COMPARE_COLORS.section.divider}`}>
        <div className={`text-xs ${BATTLE_COMPARE_COLORS.overview.label} mb-2`}>{t('battleCompare.summary')}</div>
      ...
      <div className={`relative w-full h-full sm:w-full sm:max-w-4xl sm:h-auto sm:rounded-xl ${BATTLE_COMPARE_VIEW_COLORS.modal} overflow-hidden flex flex-col`}>
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4">
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <BattleCompareSide battle={battle1} side="left" t={t} />
            <BattleCompareSide battle={battle2} side="right" t={t} />
          </div>
        </div>
        ...
        <div className={`shrink-0 ${BATTLE_DETAIL_COLORS.section.bg} p-3 sm:p-4 border-t ${BATTLE_DETAIL_COLORS.section.border}`}>
          <div className="flex justify-center">
            ...
            >
              {t('battleCompare.closeComparison')}
            </button>
          </div>
        </div>
      </div>
