...
const JAPAN_NAMES = new Set(['平安时代', '鎌倉时代', '江户时代', '飞鸟时代', '室町时代', '绳文时代', '弥生时代', '大和时代', '奈良时代', '明治时代']);
...
const WEST_NAMES = new Set(['罗马', '拜占庭', '奥斯曼', '波斯', '阿契美尼德', '帕提亚', '萨珊', '萨法维', '亚历山大', '帖木儿', '阿拔斯', '倭马亚', '古埃及', '托勒密埃及', '亚述', '巴比伦', '古巴比伦', '阿卡德', '赫梯', '迦太基']);
...
const JAPAN_NAMESMatches = ['大和', '鎌倉', '鎌倉', '平安', '江戸', '江户', '飞鸟', '室町', '绳文', '弥生', '弥生', '奈良', '明治', '戦国'];
const VIETNAM_NAMESMatches: string[] = ['丁', '前黎', '陳', '后黎'];
const CENTRAL_ASIA_NAMESMatches: string[] = ['贵霜', '笈多', '希腊-巴克特里亚', '萨塔瓦哈纳', '印度', '突厥', '德里', '毗奢耶'];
const SOUTHEAST_ASIA_NAMESMatches: string[] = ['室利佛逝', '朱罗', '高棉'];
const WEST_NAMESMatches = ['罗马', '拜占庭', '奥斯曼', '波斯', '阿契美尼德', '米底', '帕提亚', '萨珊', '萨法维', '亚历山大', '帖木儿', '阿拔斯', '倭马亚', '塞琉古', '亚述', '巴比伦', '新巴', '阿卡德', '赫梯', '神圣', '继业者', '托勒密', '基辅', '迦太基'];
...
export function classifyRegion(boundary: WorldBoundary): RegionId {
  ...
  // East Africa: Aksum
  if (name.startsWith('阿克苏姆')) return 'other';
  return 'other';
}
                    ...
                    >
                      <div className="w-4 text-[8px] text-zinc-400 text-right pr-0.5 leading-none">
                        {year === 0
                          ? '1c CE'
                          : year > 0
                            ? `${Math.floor((year - 1) / 100) + 1}c CE`
                            : `${Math.ceil((-year) / 100)}c BCE`}
                      </div>
