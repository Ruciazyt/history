 ...
 * @param year - Negative numbers are BCE, positive are CE. Year 0 is treated as 0 CE.
...
export function formatYear(year: number, locale: string = 'en'): string {
  if (year === 0) {
    return locale === 'zh' || locale === 'ja' ? '公元0年' : '0 CE';
  }
  if (year < 0) {
    const abs = Math.abs(year);
    return locale === 'zh' || locale === 'ja' ? `公元前${abs}年` : `${abs} BCE`;
  }
  return locale === 'zh' || locale === 'ja' ? `公元${year}年` : `${year} CE`;
}
